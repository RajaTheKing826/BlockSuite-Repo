...
import { CardBlockComponent } from '../schema/Card'
...
export async function mountDefaultDocEditor(
   ...
   editor.edgelessSpecs = [...EdgelessEditorBlockSpecs].map(spec => {
      if (spec.schema.model.flavour === 'affine:page') {
         spec = {
            ...
            setup: (slots, disposable) => {
               slots.mounted.once(() => {
                  const onFormatBarConnected = slots.widgetConnected.on(
                     view => {
                        if (view.component instanceof AffineFormatBarWidget) {
                           configureFormatBar(view.component)
                        }
   ...
   const cardBlockSpec = {
      ...
      view: {
         component: literal`affine-card-block`
      }
   }
   ...
   editor.edgelessSpecs = [
      ...filteredEdgelessEditorBlockSpecs,
      s3ImageSpec,
      cardBlockSpec
   ]
   ...
   const cardBlock = new CardBlockComponent()
